...
public class JdbcMetaData implements MetaData {
	...
	private static final String CREATE_PARTITIONS_TABLE = String.format(
	...
	private static final String CREATE_COLUMNS_TABLE = String.format(
			"create table if not exists %s\n" +
					"(\n" +
					"`table` varchar(100),\n" +
					"`name` varchar(100),\n" +
	...
	private static final String GET_PARTITIONS = String.format("select * from %s where srcTable = :srcTable order by id", PARTITIONS_TABLE_NAME);
	private static final String ADD_PARTITION = String.format("insert into %s (srcTable, tableName, id, entries) values (:srcTable, :tableName, :id, :entries)", PARTITIONS_TABLE_NAME);
	private static final String ADD_COLUMN = String.format("insert into %s (`table`, `name`, `min`, `max`) values (:table, :name, :min, :max)", COLUMNS_TABLE_NAME);
	private static final String GET_COLUMN = String.format("select * from %s where `table` = :table and `name` = :name", COLUMNS_TABLE_NAME);
